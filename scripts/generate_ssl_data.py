...
assert sys.implementation.name == 'cpython', "This script is meant to be run with CPython"

import jinja2

def get_openssl_prefix():
    import _ssl
    ssl_deps = subprocess.check_output(['ldd', _ssl.__file__], universal_newlines=True)
    for line in ssl_deps.splitlines():
        items = line.split()
        if len(items) == 4 and items[0].startswith('libssl.so'):
            libdir = os.path.dirname(os.path.abspath(items[2]))
            prefix = os.path.dirname(libdir)
            if prefix == '/':
                print("WARNING: this script is meant to be executed by the CPython build used in CI")
                return '/usr'
            return prefix
    sys.exit("Couldn't determine OpenSSL prefix directory")

OPENSSL_PREFIX = get_openssl_prefix()
OPENSSL_BINARY = os.path.join(OPENSSL_PREFIX, 'bin', 'openssl')
...
def get_all_ciphers():
    ...
    standard_names = {}
    cmd = [OPENSSL_BINARY, 'ciphers', '-stdname', cipher_string]
...
def get_cipher_strings_mapping(all_ciphers):
    ...
    for cipher_string in get_all_cipher_strings():
        ...
        cmd = [OPENSSL_BINARY, 'ciphers', '-stdname', '-ciphersuites', '', cipher_string]
        ciphers = []
...
openssl_version = subprocess.run([OPENSSL_BINARY, 'version'], capture_output=True, text=True, check=True).stdout
autogenerated = f"""\
...
subprocess.run(
    ['mx', 'eclipseformat', '--primary', '--filelist', '-'],
    text=True,
