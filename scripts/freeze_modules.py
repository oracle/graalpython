...
def freeze_module(src):
    with open(src.pyfile, 'r') as src_file, open(src.frozenfile, 'w') as frozen_file:
        ...
        frozen_file.write("package com.oracle.graal.python.frozen;\n\n")

        frozen_file.write(f"public final class Frozen{src.id.title()}{{\n")
        frozen_file.write(f"public static final byte[] {src.id}ByteCode = {{\n")
        _write_bytes(frozen_file, bytecode)
        frozen_file.write("};\n}")
...
def main():
    ...
    for src in _iter_sources(modules):
        freeze_module(src)
    # iterate module specs call #freeze_module
        # freezes module to bytecode and writes to to file
    # update imports file from module specs -> where does it need to be included?
