...
def build_java_generator(
    ...
    with open(tokens_file, "r") as tok_file:
        all_tokens, exact_tok, non_exact_tok = generate_token_definitions(tok_file)
    with open(output_file, "w") as file:
        gen: ParserGenerator = JavaParserGenerator(grammar, all_tokens, exact_tok, non_exact_tok, file)  # TODO: skip_actions
        gen.generate(grammar_file)
    return gen
...
def build_java_parser_and_generator(
    ...
    """
    grammar, parser, tokenizer = build_parser(grammar_file, verbose_tokenizer, verbose_parser)
    gen = build_java_generator(grammar, grammar_file, tokens_file, output_file, skip_actions=skip_actions,)
    return grammar, parser, tokenizer, gen
