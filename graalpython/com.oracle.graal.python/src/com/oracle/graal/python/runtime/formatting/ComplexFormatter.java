...
public class ComplexFormatter extends InternalFormat.Formatter {
    ...
    protected ComplexFormatter(PythonCore core, FormattingBuffer result, Spec spec) {
        ...
        // Turn off any flags that should apply to the result as a whole and not to the individual
        // components (re/im). Sign of re is determined by the sign flag, sign of im will be always
        // printed ('+' flag)
        reFormatter = new FloatFormatter(core, result, getComponentSpec(spec, spec.sign));
        imFormatter = new FloatFormatter(core, result, getComponentSpec(spec, '+'));
    }
