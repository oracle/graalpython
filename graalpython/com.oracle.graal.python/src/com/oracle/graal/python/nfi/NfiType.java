...
public enum NfiType {
    ...
    public Object convertArg(Object arg) throws UnsupportedTypeException {
        return switch (this) {
            case VOID -> throw shouldNotReachHere("VOID cannot be an argument");
            case POINTER -> unwrapPointer(arg);
            default -> {
                if (!asJavaType().isInstance(arg)) {
                    throw shouldNotReachHere("Expected " + asJavaType().getName() + ", got " + arg.getClass() + "(" + arg + ") instead");
                }
                yield arg;
            }
        };
    ...
    private static long unwrapPointer(Object arg) throws UnsupportedTypeException {
        InteropLibrary interop = InteropLibrary.getUncached(arg);
        try {
