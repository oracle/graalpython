...
public abstract class PBytecodeDSLRootNode extends PRootNode implements BytecodeRootNode {
    ...
    public static final class MakeDict {
        // TODO: split to empty dict and non-empty dict created directly from preallocated hashmap
        // storage
        @Specialization
        @ExplodeLoop
    ...
    public static final class UnpackToLocals {
        ...
        public static void doUnpackIterable(VirtualFrame virtualFrame, LocalRangeAccessor results, Object collection,
            ...
            for (int i = 0; i < count; i++) {
                try {
                    results.setObject(bytecode, virtualFrame, i, getNextNode.execute(virtualFrame, inliningTarget, iterator));
                } catch (IteratorExhausted e) {
    ...
    public static final class Le {
        ...
        public static final Object doGeneric(VirtualFrame frame, Object left, Object right,
                        @Bind PBytecodeDSLRootNode rootNode,
                        @Bind Node inliningTarget,
                        @Cached InlinedConditionProfile notSupportedProfile,
                        @Cached("createCallNode()") LookupAndCallBinaryNode callNode) {
            Object result = callNode.executeObject(frame, left, right);
            if (notSupportedProfile.profile(inliningTarget, result == PNotImplemented.NOT_IMPLEMENTED)) {
                throw notSupported(left, right, rootNode, T_OPERATOR);
            }
            return result;
        }
    ...
    public static final class UnpackSequence {
        ...
        public static Object[] doUnpackIterable(VirtualFrame virtualFrame,
            ...
            for (int i = 0; i < count; i++) {
                try {
                    result[i] = getNextNode.execute(virtualFrame, inliningTarget, iterator);
                } catch (IteratorExhausted e) {
                    throw raiseNode.raise(inliningTarget, ValueError, ErrorMessages.NOT_ENOUGH_VALUES_TO_UNPACK, count, i);
                }
            }
    ...
    public static final class UnpackEx {
        ...
        private static void copyItemsToArray(VirtualFrame frame, Node inliningTarget, Object iterator, Object[] destination, int destinationOffset, int length, int totalLength,
            ...
            for (int i = 0; i < length; i++) {
                try {
                    destination[destinationOffset + i] = getNextNode.execute(frame, inliningTarget, iterator);
                } catch (IteratorExhausted e) {
    ...
    public static final class YieldFromSend {
        ...
        static boolean doIterator(VirtualFrame virtualFrame,
            ...
            try {
                ...
                // TODO: construct directly, not raise and catch
                try {
                    throw PRaiseNode.raiseStatic(inliningTarget, PythonBuiltinClassType.StopIteration);
                } catch (PException pex) {
                    handleException(virtualFrame, pex, inliningTarget, bytecode, stopIterationProfile, getValue, returnedValue);
                    return true;
                }
            }
        }
