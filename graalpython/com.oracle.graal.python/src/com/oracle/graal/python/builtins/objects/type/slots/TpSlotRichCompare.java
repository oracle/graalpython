...
public abstract class TpSlotRichCompare {
    /**
     * Mirror of the CPython constants of the same name. The order is important: the ordinal should
     * be the same as the constant value on the CPython side.
     */
    public enum RichCmpOp {
        ...
        static RichCmpOp fromName(TruffleString name) {
         ...
         * {@link Integer#compare(int, int)} to a boolean value according to the given operation. If
         * the result is computed, make sure that it can never overflow!
         */
    ...
    public static final class TpSlotRichCmpPython extends TpSlotPython {
        ...
        private static boolean checkCallableNames(TruffleString[] callableNames) {
            for (int i = 0; i < RichCmpOp.VALUES.length; i++) {
                assert RichCmpOp.VALUES[i].pythonName.equalsUncached(callableNames[i], TS_ENCODING);
            }
            return true;
        }
