...
@CoreFunctions(defineModule = "_codecs_kr")
public class CodecsKRModuleBuiltins extends PythonBuiltins {
    ...
    protected static boolean setCodecs(String name, TruffleString tsName, Charset charset) {
        ...
        if (name.contentEquals("ksx1001")) {
            MAPPING_LIST[0] = new DBCSMap(name, tsName, charset, MappingType.DECONLY);
            return true;
        }
        if (name.contentEquals("cp949ext")) {
            MAPPING_LIST[2] = new DBCSMap(name, tsName, charset, MappingType.DECONLY);
            return true;
        }
        return false;
    }
