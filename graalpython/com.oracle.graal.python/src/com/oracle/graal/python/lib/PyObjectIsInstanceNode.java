...
/**
 * Equivalent of CPython's {@code PyObject_IsInstance}. Implements tuple recursion for
 * {@code classinfo} and the {@code __instancecheck__} fallback protocol.
 */
@GenerateInline(false)
@GenerateUncached
@ImportStatic(PGuards.class)
public abstract class PyObjectIsInstanceNode extends PyObjectRecursiveBinaryCheckNode {
    @Override
    ...
    @NeverDefault
    public static PyObjectIsInstanceNode create() {
        return PyObjectIsInstanceNodeGen.create();
    }

    public static PyObjectIsInstanceNode getUncached() {
    ...
    static boolean isInstance(VirtualFrame frame, Object instance, Object cls, @SuppressWarnings("unused") int depth,
                    ...
                    @Cached LookupSpecialMethodNode.Dynamic instanceCheckLookup,
                    @Cached CallBinaryMethodNode callInstanceCheck,
        ...
        if (classProfile.profileClass(inliningTarget, getClsClassNode.execute(inliningTarget, cls), PythonBuiltinClassType.PythonClass)) {
            // Avoid the lookup and call overhead when we know we're calling type.__instancecheck__.
            return genericInstanceCheckNode.execute(frame, inliningTarget, instance, cls);
        }
        Object method = instanceCheckLookup.execute(frame, inliningTarget, getClsClassNode.execute(inliningTarget, cls), T___INSTANCECHECK__, cls);
        if (PGuards.isNoValue(method)) {
            noInstanceCheckProfile.enter(inliningTarget);
            return genericInstanceCheckNode.execute(frame, inliningTarget, instance, cls);
        }
        Object result = callInstanceCheck.executeObject(frame, method, cls, instance);
