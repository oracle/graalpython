...
public class PythonInlineTreeTranslator extends PythonBaseTreeTranslator<PNode> {

    private PNode result;

    public PythonInlineTreeTranslator(PythonCore core, String name, ParserRuleContext input, TranslationEnvironment environment, Source source) {
        super(core, name, environment, source);

        try {
            Object parseResult = input.accept(this);
            if (parseResult instanceof PNode) {
                result = (PNode) parseResult;
            } else {
    ...
    @Override
    public PNode getTranslationResult() {
        return result;
    }
    ...
    public Object visitSingle_input(Python3Parser.Single_inputContext ctx) {
        ...
        PNode body = asBlockOrPNode(visitChildren(ctx));
