...
public class SSTNodeUtils {
     ...
     */
    public static FunctionDefSSTNode createFunctionDefWithArguments(String name, ScopeInfo functionScope, SSTNode body, String[] arguments) {
        ArgDefListBuilder argBuilder = new ArgDefListBuilder();
        ...
        // statement
        if (body instanceof BlockSSTNode) {
            BlockSSTNode block = (BlockSSTNode) body;
            int index = block.statements.length - 1;
            SSTNode lastStatement = block.statements[index];
            if (lastStatement instanceof ExpressionStatementSSTNode) {
                SSTNode expression = ((ExpressionStatementSSTNode) lastStatement).expression;
