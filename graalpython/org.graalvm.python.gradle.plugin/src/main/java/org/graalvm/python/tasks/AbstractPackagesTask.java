...
public abstract class AbstractPackagesTask extends DefaultTask {
    ...
    protected static final String REQUIREMENTS_FILE_HEADER = """
        This file was generated by gradle task 'graalpyFreezeInstalledPackages'.
    ...
    protected static final String PACKAGE_REMOVED_ERROR = """
        Install of python packages is based on requirements file %s,
        but some packages in graalpy-gradle-plugin configuration were removed since previously used to generate the requirements file.
    ...
    protected Path getRequirementsPath() {
        Path rfp = getRequirementsFile().get().getAsFile().toPath();
        if(rfp.isAbsolute()) {
            return rfp;
        } else {
            return getProject().file(getRequirementsFile().get()).toPath();
        }
    }
}
