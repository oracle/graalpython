...
from utils import get_pickler, random_instance
...
def __setup__(size=100, module="pickle"):
    global DATA, dumps, loads
    dumps, loads = get_pickler(module)
    DATA = [random_instance() for _ in range(size)]


def __process_args__(size=100, module="pickle"):
    return int(size), module
...
def __benchmark__(size=100, module="pickle"):
