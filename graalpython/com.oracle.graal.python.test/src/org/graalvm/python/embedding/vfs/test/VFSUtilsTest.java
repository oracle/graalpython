...
public class VFSUtilsTest {
    ...
    private static final String PACKAGES_CHANGED_ERROR = "packages changed requirements file %s, current packages %s, previous packages %s";
    ...
    /**
     * tests scenarios when packages are declared only in plugin config, and requirement file logic
     * is available, even though no used
     */
    @Test
    ...
    public void withoutRequirementsFile() throws IOException {
        ...
        // new graalPy version
        createVenv(venvDir, "0.2", log, "hello-world==0.1");
        checkVenvCreate(log.getOutput(), true);
        assertTrue(log.getOutput().contains(STALE_VENV));
        checkInstalledPackages(venvDir.resolve("installed.txt"), "hello-world");
        checkVenvContentsFile(contents, "0.2", "hello-world==0.1");
        log.clearOutput();
    }
    ...
    public void missingRequirementsWarning() throws IOException {
        TestLog log = new TestLog();
        Path tmpDir = Files.createTempDirectory("missingRequirementsWarning");
        Path venvDir = tmpDir.resolve("venv");
        Path contents = venvDir.resolve("contents");
        deleteDirOnShutdown(tmpDir);
        ...
        // install request, it pulls in transitive pkgs and
        ...
        // 1.) first freeze requests which pulls in transitive deps
        freeze(venvDir, requirements, log, "requests==2.32.3", "hello-world==0.2");
        checkVenvCreate(log.getOutput(), false);
        checkRequirementsFile(requirements, "requests==2.32.3", "charset-normalizer", "idna", "urllib3", "certifi", "hello-world==0.2");
        checkInstalledPackages(venvDir.resolve("installed.txt"), "requests==2.32.3", "charset-normalizer", "idna", "urllib3", "certifi", "hello-world==0.2");
        checkVenvContentsFile(contents, "0.1", "requests==2.32.3", "hello-world==0.2");
        log.clearOutput();
        // 2.) then freeze without it - its transitive deps have to be gone as well
    ...
    public void installAndFreeze() throws IOException {
        ...
        // we changed hello-world from 01. to 0.2, we do not know if the prev version did not leave
        ...
        // remove tiny-tiny from packages list - fails because it is still in requirements
        checkException(IOException.class, () -> createVenv(venvDir, "0.1", log, requirements, "hello-world==0.2"));
        checkVenvCreate(log.getOutput(), false);
        assertFalse(log.getOutput().contains("pip install"));
        assertTrue(log.getOutput().contains(String.format(PACKAGES_CHANGED_ERROR, requirements, "hello-world==0.2", "hello-world==0.2, tiny-tiny==0.2")));
        log.clearOutput();
        // freeze requirements with new hello-world
        freeze(venvDir, requirements, log, "hello-world==0.2");
        checkVenvCreate(log.getOutput(), true);
        ...
        createVenv(venvDir, "0.2", log, requirements, "hello-world==0.2");
        checkVenvCreate(log.getOutput(), true);
        assertTrue(log.getOutput().contains(STALE_VENV));
        assertTrue(log.getOutput().contains("pip install -r")); // requirements file is used
        checkInstalledPackages(venvDir.resolve("installed.txt"), "hello-world==0.2");
        checkVenvContentsFile(contents, "0.2", "hello-world==0.2");
        assertFalse(log.getOutput().contains(MISSING_REQUIREMENTS_FILE_WARNING));
        log.clearOutput();
    }
