...
public class VFSUtilsTest {
    ...
    private static void lock(Path venvDir, Path lockFile, TestLog log, String... packages) throws IOException {
        VFSUtils.lockPackages(venvDir, Arrays.asList(packages), lockFile, LOCK_FILE_HEADER, WRONG_PKG_VERSION_ERROR, createLauncher(venvDir), "0.1", log);
    }
    ...
    public void installAndLock() throws IOException {
        ...
        // lock without version - fails
        checkException(IOException.class, () -> lock(venvDir, lockFile, log, "requests"));
        assertFalse(Files.exists(lockFile));
        assertTrue(log.getOutput().contains(String.format(WRONG_PKG_VERSION_ERROR, "requests")));
        assertFalse(log.getOutput().contains(MISSING_LOCK_FILE_WARNING));
        // lock with version - ok
    ...
    private static void createVenv(Path venvDir, String graalPyVersion, TestLog log, Path lockFile, String... packages) throws IOException {
        EmbeddingTestUtils.createVenv(venvDir, graalPyVersion, log, lockFile, LOCK_FILE_HEADER, WRONG_PKG_VERSION_ERROR, MISSING_LOCK_FILE_WARNING, PACKAGES_CHANGED_ERROR,
                        packages);
    }
