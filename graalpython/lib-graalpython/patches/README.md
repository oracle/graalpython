...
install-priority = 1
# Optional. Whether to run `autopatch_capi` and `autopatch_cargo` on a source distribution before applying the
# explicit patch. Autopatching is never applied to wheels. Defaults to true.
autopatch = true
