...
_Py_IDENTIFIER(cached_datetime_module);

static PyObject *
get_interp_key(void)
{
    return _PyUnicode_FromId(&PyId_cached_datetime_module);
}
...
get_current_module(PyInterpreterState *interp, int *p_reloading)
{
    ...
    // GraalPy change: avoid _Py_ID
    PyObject *interp_key = get_interp_key();
    if (interp_key == NULL) {
        goto error;
    }
    PyObject *ref = NULL;
...
// GraalPy change: our normal datetime.h macros handle managed objects, redefine to handle only native
