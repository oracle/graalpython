...
// GraalPy-specific forward declaration
static inline void _decref_notify(const PyObject *op, const Py_ssize_t updated_refcnt);
...
_Py_DecRef(PyObject *o)
{
    ...
    if (refcnt != _Py_IMMORTAL_REFCNT) {
        ...
        if (updated_refcnt != 0) {
            _decref_notify(o, updated_refcnt);
        }
        else {
            _Py_Dealloc(o);
        }
...
#if 0 // GraalPy change: this fast path is only applicable to cpython
    ...
    if (dict == NULL) {
        if ((tp->tp_flags & Py_TPFLAGS_INLINE_VALUES)) {
            if (PyUnicode_CheckExact(name) &&
                ...
                // GraalPy change: inline values are not materialized in cext.
...
_Py_SetRefcnt(PyObject *ob, Py_ssize_t refcnt)
{
    ...
    PyObject *dest;
    if (points_to_py_handle_space(ob)) {
        dest = pointer_to_stub(ob);
    } else {
        dest = ob;
    }
    dest->ob_refcnt = refcnt;
}
...
static PyObject* constants[] = {
    NULL,  // Py_CONSTANT_NONE
    NULL,  // Py_CONSTANT_FALSE
    NULL,  // Py_CONSTANT_TRUE
    NULL,  // Py_CONSTANT_ELLIPSIS
    NULL,  // Py_CONSTANT_NOT_IMPLEMENTED
...
{
    constants[Py_CONSTANT_NONE] = &_Py_NoneStruct;
    constants[Py_CONSTANT_FALSE] = (PyObject*)(&_Py_FalseStruct);
    constants[Py_CONSTANT_TRUE] = (PyObject*)(&_Py_TrueStruct);
    constants[Py_CONSTANT_ELLIPSIS] = &_Py_EllipsisObject;
    constants[Py_CONSTANT_NOT_IMPLEMENTED] = &_Py_NotImplementedStruct;
