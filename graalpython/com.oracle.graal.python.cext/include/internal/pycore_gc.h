...
static inline void _PyGCHead_SET_NEXT(PyGC_Head *gc, PyGC_Head *next) {
    _Py_RVALUE(_PyGCHead_UNTAG(gc)->_gc_next = (uintptr_t)next);
}
...
#else
    PyGC_Head *gc = _PyGCHead_UNTAG(_Py_AS_GC(op));
    _Py_RVALUE(gc->_gc_prev |= _PyGC_PREV_MASK_FINALIZED);
#endif
}
...
#define _PyGCHead_FINALIZED(gc) \
    ((_PyGCHead_UNTAG(gc)->_gc_prev & _PyGC_PREV_MASK_FINALIZED) != 0)
...
extern PyAPI_FUNC(void) _PyGC_CollectNoFail(PyThreadState *tstate);
